...
class Pose:
    ...
    def check_pose_LRC(self, image, results, draw=False, display=False):
        """
        ...
        left_x = int(
            results.pose_landmarks.landmark[self.mp_pose.PoseLandmark.RIGHT_SHOULDER].x
            * width
        )

        # Retreive the x-corrdinate of the right shoulder landmark.
        right_x = int(
            results.pose_landmarks.landmark[self.mp_pose.PoseLandmark.LEFT_SHOULDER].x
            * width
        )
    ...
    def check_pose_JSD(self, image, results, MID_Y=250, draw=False, display=False):
        """
    ...
    def check_hands_joined(self, image, results, draw=False, display=False):
        """
        ...
        left_wrist_landmark = (
            results.pose_landmarks.landmark[self.mp_pose.PoseLandmark.LEFT_WRIST].x
            * width,
        ...
        right_wrist_landmark = (
            results.pose_landmarks.landmark[self.mp_pose.PoseLandmark.RIGHT_WRIST].x
            * width,
